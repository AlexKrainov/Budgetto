...
Vue.component("vue-select2-component", {
	...
	props: { //transferred variables
		...
		dataClass: {
			type: String,
			default: "form-control select2-multiple select2-hidden-accessible" //form-control crm-crash-list-contract-filter"
		},
		language: {
			type: String
		},
	...
	mounted: function () { //method starts after loading
		...
		if (!this.language) {//if someone was set to language some value, we should use it, but if language is empty we use by user localization
			switch (document.LocalizationLanguage.CodeName) {
				case "ItalyItalian":
					this.language = "it";
					break;
				default:
		...
		let select2_obj = $(this.$el).select2({
			ajax: this.items != undefined ? null : {
				...
				data: function (params) {
					...
					for (var prop in searchParams) { //remove all property with null and undefined
						if (searchParams[prop] == null || searchParams[prop] == undefined) {
				...
				processResults: function (data, params) {
					...
					return {
						results: data.Items,
						pagination: {
							more: (params.page * 20) < data.TotalItemCount
						}
					};
				},
				cache: true
	...
	methods: {
		...
		templateResult: function (repo) { //show values in option elements
			...
			for (var i = 0; i < this.metadata.length; i++) {
				if (this.metadata[i].isOption == true && repo[this.metadata[i].propertyName]) {
					markup += "<div class='select2-result-block'>" + (this.metadata[i].text ? ("<span class='select2-option-title'>" + this.metadata[i].text + ":</span> ") : "") + "<span class='select2-option-value'>" + repo[this.metadata[i].propertyName] + "</span></div>";
				}
			}
			markup += "</div>";
			return markup;
